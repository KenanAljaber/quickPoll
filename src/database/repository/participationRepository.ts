...
export default class ParticipationRepository {
  ...
  static async create(
    ...
    await this.checkVoteIpAddressHistory(data.ipAddress, vote.id, options);
    if (data.optionsId) {
      const optionsWithVoteId = data.optionsId.map((option) => ({
        optionId: option,
        voteId: vote.id,
      }));
      const result = await options.database.optionVotes.bulkCreate(
        optionsWithVoteId,
        {
          ...options,
          transaction,
  ...
  static async checkVoteIpAddressHistory(ipAddress: string, voteId: string, options: IRepositoryOptions) {
    const transaction = await SequelizeRepository.getTransaction(options);
  ...
  static async checkPollOptionsIntegrity(
    ...
    if (!poll.isMultiAnswer && data.optionsId.length > 1)
